...
//Util interface를 구현
public class Manager_v6 {
	private static ArrayList<PhoneInfor_v6> pblist;
	
	//인스턴스 직접참조 및 다수의 인스턴스 생성을 막기 위한 singleton pattern 적용 
	//s1) 생성자 접근제한  
	private Manager_v6 () {
		pblist = new ArrayList<PhoneInfor_v6>();
	}
	
	//s2) class 내부변수를 통한 Manager instance 생성 
	//---> getInstance를 통해서 호출될 수 있도록 static 선언
	private static Manager_v6 manager = new Manager_v6();
	
	//s3) 외부접근을 가능하게 해주는 getInstance 생성 
	public static Manager_v6 getInstance() {
		if(manager==null) {
			manager = new Manager_v6();
		}
		return manager;
	}
	...
	public static int startMenu() {
		...
		System.out.printf("%d) 신규정보등록 \n%d) 기존정보검색 \n%d) 기존정보삭제 \n%d) 기존정보수정  \n%d) 전체정보보기 \n%d) 기본정보보기 \n%d) 나가기\n", Util.INSERT,
				Util.SEARCH, Util.DELETE, Util.EDITINFO, Util.ALLINFO, Util.BASICINFO, Util.END);
		System.out.println("**********************************");
